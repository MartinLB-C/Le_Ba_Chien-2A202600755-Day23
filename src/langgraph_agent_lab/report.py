...
def render_report(metrics: MetricsReport) -> str:
    ...
    for sm in metrics.scenario_metrics:
        ...
        report_lines.append(
            f"| {sm.scenario_id} | {sm.expected_route} | {actual_route} | "
            f"{success_str} | {sm.retry_count} | {sm.nodes_visited} |"
        )
    ...
    report_lines.extend(
        [
            ...
            (
                "The graph is a `StateGraph` consisting of 11 distinct nodes. "
                "The `AgentState` uses `TypedDict` for mutable routing fields "
                "(`route`, `attempt`, `evaluation_result`) and "
